...
def start_servers():
    ...
    time.sleep(5)  # Adjust based on app startup time
    ...
    try:
        ...
        requests.get(f"http://localhost:{FASTAPI_PORT}", timeout=5)
        requests.get(f"http://localhost:{STREAMLIT_PORT}", timeout=5)
